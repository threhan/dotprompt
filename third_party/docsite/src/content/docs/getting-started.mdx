...
const ai = genkit({
  plugins: [googleAI()],
  // promptDir: 'prompts', /* this is the default */
});
...
With this setup, you can now create `.prompt` files in a `prompts` directory in your project and use them in your code. For example, if you have a file named `extractInfo.prompt` with the content from the earlier example, you can use it like this:

```typescript
const extractInfoPrompt = ai.prompt('extractInfo');

const {output} = await extractInfoPrompt({
